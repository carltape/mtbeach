...
$Brect = "-Bxa${xtick1}f${xtick2}g${xtick1} -Bya${ytick1}f${ytick2}g${ytick1} -Bwesn";  # 6.0.1
...
$cshfile = "lune_gmt601.csh";
open(CSH,">$cshfile");
# set GMT default parameters
print CSH "gmt gmtset PS_MEDIA letter PS_PAGE_ORIENTATION landscape PROJ_LENGTH_UNIT inch MAP_FRAME_TYPE plain MAP_TICK_LENGTH $ticklen MAP_FRAME_PEN $fpen MAP_TICK_PEN $tpen FONT_ANNOT_PRIMARY 12p,Helevetica,black FONT_HEADING 18p,Helvetiva,black FONT_LABEL 10p,Helvetiva,black\n";
...
if($k==1) {
  ...
  print CSH "gmt psbasemap $J $R ${B}+g$clune -K -O -V -X5 >> $psfile\n";
  $ilegend = 0;
}
...
if ($plot_ref_points || $plot_ref_labels) {
  ...
  #if ($plot_ref_points) {
  #  $csize = 12;
  #  print CSH "gmt psxy $fname -N -Sc${csize}p -W1p,0/0/0 -G255 -J -R -K -O -V >>$psfile\n";
  #}
...
# print CSH "gmt psxy -J -R -N $pinfo -K -O -V >>$psfile<<EOF
#  0 0
# -2.747 9.619
# -5.657 19.215
# -8.93 28.759
...
if ($iplot==1) {
  ...
  #@inds = 14;
        ...
        for ($i = 1; $i <= @inds; $i++) {
          ...
          #print CSH "gmt psxy $fname -N -Sc${cz}p -W0.5p,0/0/0 -G$cols[$j-1] -J -R -K -O -V >>$psfile\n";
          print CSH "awk '{print \$${col1},\$${col2}}' $fname | gmt psxy -N -Sc${cz}p -W0.5p,0/0/0 -G$cols[$j-1] -J -R -K -O -V >> $psfile\n";
        }
...
#       print CSH "awk '{print \$8,\$9}' $fname1 | gmt psxy -N -Sc${csize}p -W0.5p,0/0/0 -G$cols[0] -J -R -K -O -V >> $psfile\n";
#       print CSH "awk '{print \$8,\$9}' $fname2 | gmt psxy -N -Sc${csize}p -W0.5p,0/0/0 -G$cols[1] -J -R -K -O -V >> $psfile\n";
   ...
   if(0==1) {
       ...
       print CSH "awk '{print \$1,\$2,\$3}' dtemp | gmt psxy -N -Sc${csize}p -W0.5p,0/0/0 -C$cptfile -J -R -K -O -V >> $psfile\n";
       $Dscale = "-D0/1/2/0.2";
       $Bscale = "-B10f5:\"F-test significance\": -Eb10p";
       print CSH "gmt psscale -C$cptfile $Dscale $Bscale -Xa3 -Ya5.5 -V -K -O >> $psfile\n";
   }
  ...
  if($icat==1) {        # Ford2009 -- Nevada Test Site
    ...
    print CSH "awk '{print \$1,\$2}' $farc1 | gmt psxy -W${arcwid}p,0/255/255 -J -R -K -O -V >>$psfile\n";
    print CSH "awk '{print \$1,\$2}' $farc2 | gmt psxy -W${arcwid}p,255/0/255 -J -R -K -O -V >>$psfile\n";
  }
  if (not -f $cmtfile) {die("\n check if cmt file $cmtfile exists\n");}

  $cmtinfo = "-Sm0.3 -L0.5p,0/0/0 -N";
  if ($icat==6) {
      print CSH "gmt psmeca $cmtfile $J $R $cmtinfo -G255/0/0 -K -O -V >> $psfile\n";

  } else {
      $cptfile = "color.cpt";
      print CSH "makecpt -C$cpal -T$Mran -D > $cptfile\n";
      print CSH "gmt psmeca $cmtfile $J $R $cmtinfo -Z$cptfile -K -O -V >> $psfile\n";
      $Dscale = "-D3/7/1.5/0.25";
      $Bscale = "-B${Mtick1}f${Mtick2}g${Mtick2}:\"$ttag\": -Al";
      if ($k==1) {
          print CSH "gmtset TICK_LENGTH 8p\n";
	  print CSH "gmt psscale -C$cptfile $Dscale $Bscale -V -K -O >> $psfile\n";
...
if($iplot==1 && $ilegend==1) {
  ...
  for ($i = 1; $i <= @inds; $i++) {
    ...
    print CSH "gmt psxy -N -Sc${cz}p -W0.5p,0/0/0 -G$cols[$j-1] $R_title $J_title $olegend -K -O -V >>$psfile<<EOF\n$x0 $y\nEOF\n";
    print CSH "gmt pstext -N $R_title $J_title $olegend -K -O -V >>$psfile<<EOF\n $x $y 12 0 $fontno LM $lab\nEOF\n";
  }

#$x = -30; $y = rad2deg(asin(1/sqrt(3)));
#print CSH "gmt psxy -N -Sc${csize}p -W1p,0/0/0 -G255/165/0 -J -R -K -O -V >>$psfile<<EOF\n$x $y\nEOF\n";
}
...
if ($ititle==1) {
  ...
  print CSH "gmt pstext -N $R_title $J_title $otitle2 $xtag -O -V >>$psfile<<EOF\n 0 0 $fsize2 0 $fontno CM $title2\nEOF\n";

} else {
  # any command with no -K should work
  print CSH "gmt psxy -R -J $xtag -O -T -V >> $psfile\n";
}
