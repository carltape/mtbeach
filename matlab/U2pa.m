...
if itype==1
    ...
    ipos3 = find(p3(3,:) < 0);
    [~,~,n] = size(Uin);
    disp(sprintf('U2pa: %i/%i p1 vectors need to be flipped',length(ipos1),n));
    disp(sprintf('U2pa: %i/%i p2 vectors need to be flipped',length(ipos2),n));
    disp(sprintf('U2pa: %i/%i p3 vectors need to be flipped',length(ipos3),n));
